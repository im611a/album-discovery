...
8. 正常动效证据与稳定像素基准分开：像素基准禁用动画，动效测试采集关键帧、状态值
   或 WebM，并验证滚动反向与指针静止。
9. 全站验收使用页面原型矩阵；动态路由抽查重点、普通、长标题和缺失字段场景。
10. 读取实际元素矩形检查非法重叠，并扫描静态 HTML 的内部链接和资源。
...
- 报告浏览器、视口、路由、截图/录屏、几何重叠、console/network、状态兼容与失败场景。
