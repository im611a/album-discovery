...
const commands = new Set(["doctor", "acquire", "dry-run", "status", "review", "prepare", "promote", "recover", "help"]);
const booleanOptions = new Set(["json", "refresh", "verbose"]);
const valueOptions = new Set(["input", "batch", "apply", "transaction", "candidate-fingerprint", "concurrency"]);
...
function contextFromEnvironment(environment = process.env) {
  if (environment.NODE_ENV === "test" && environment.CONTENT_PIPELINE_OPERATOR_TEST_ROOT) {
    ...
    return { repositoryRoot: root, workspaceRoot: path.join(root, ".local-data", "content-pipeline-v1"), catalogPath: path.join(root, "production", "generated", "catalog.json"), generatedRoot: path.join(root, "production", "generated"), coverRoot: path.join(root, "production", "covers"), identitiesPath: path.join(root, "fixtures", "netease-identities.json"), rymSnapshotPath: path.join(root, "fixtures", "rym-taxonomy-snapshot.json") };
  }
  return { repositoryRoot, workspaceRoot: defaultWorkspaceRoot, catalogPath: path.join(repositoryRoot, "src", "data", "generated", "catalog.json"), generatedRoot: path.join(repositoryRoot, "src", "data", "generated"), coverRoot: path.join(repositoryRoot, "public", "catalog", "covers"), identitiesPath: path.join(repositoryRoot, "scripts", "catalog", "netease-identities.json"), rymSnapshotPath: path.join(repositoryRoot, "scripts", "catalog", "rym-taxonomy-snapshot.json") };
}
...
async function createBatchForInput(ctx, input) {
  ...
  config.operator = { sourceInputSha256: await sha256File(source), sourceName: path.basename(source) };
...
async function snapshotQualification(ctx, batch, result) {
  ...
  const qualification = {
    ...
    batchId: batch.id,
    inputSha256: await sha256File(path.join(batch.root, config.input)),
...
async function readQualification(ctx, batch) {
  ...
  const config = JSON.parse(await readFile(path.join(batch.root, "batch.json"), "utf8"));
  const inputSha256 = await sha256File(path.join(batch.root, config.input));
...
async function execute(options, ctx = contextFromEnvironment()) {
  ...
  if (command === "help") return envelope(command, { status: "HELP", message: "doctor | acquire --input FILE | dry-run (--input FILE|--batch ID) | status --batch ID | review --batch ID [--apply FILE] | prepare --batch ID | promote --batch ID --transaction ID --candidate-fingerprint SHA256 | recover --batch ID" });
  if (command === "doctor") {
  ...
  if (command === "review") return withLocks(lockSet(ctx, batch, command), async () => {
    ...
    if (!options.apply) { const template = reviewTemplate({ batchId: batch.id, inputSha256: qualification.inputSha256, records: qualification.report.records }); await writeFile(path.join(batch.root, "review", "review-template.json"), stableJson(template), "utf8"); return envelope(command, { status: "REVIEW_EXPORTED", batchId: batch.id, artifacts: { template: path.join(batch.root, "review", "review-template.json") }, exitCategory: template.decisions.length ? "review" : "success" }); }
    ...
    await writeFile(file, stableJson({ schema: validated.schema, batchId: validated.batchId, inputSha256: validated.inputSha256, decisions: validated.decisions }), "utf8");
    return envelope(command, { status: "REVIEW_APPLIED_QUALIFICATION_INVALIDATED", batchId: batch.id, fingerprints: { review: validated.fingerprint }, nextAction: `dry-run --batch ${batch.id}` });
  });
