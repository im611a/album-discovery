...
export default async function ArtistPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <SiteShell mainClassName="r12-artist-file-page">
    ...
    <section className="catalog-section pa-artist-catalog r12-discography" aria-labelledby="artist-albums-title"><header className="section-heading"><div><p className="section-kicker">DISCOGRAPHY / CHRONOLOGY</p><h2 id="artist-albums-title">作品年表</h2></div><p>{albums.length} 件作品 · 新到旧</p></header>
      <div className="r12-discography__timeline">{chronology.map((group) => <section key={group.year} className="r12-discography__year" aria-labelledby={`artist-year-${group.year}`}><h3 id={`artist-year-${group.year}`}>{group.year}</h3><div>{group.albums.map((album) => <article key={album.id} className="r12-discography__release"><ArtistCollectionAlbumLink albumSlug={album.slug} originArtistSlug={artist.slug} className="r12-discography__cover" aria-label={`查看《${album.title}》专辑详情`}><AlbumCover album={album} /></ArtistCollectionAlbumLink><div className="r12-discography__release-copy"><h4><ArtistCollectionAlbumLink albumSlug={album.slug} originArtistSlug={artist.slug}>{album.title}</ArtistCollectionAlbumLink></h4><p>{RELEASE_TYPE_LABELS[album.albumType]}{album.releaseDate ? ` · ${album.releaseDate}` : " · 发行日期暂缺"}</p>{album.coreGenres.length ? <p>{album.coreGenres.slice(0, 3).map(getTaxonomyLabel).join(" · ")}</p> : null}</div><ArtistAlbumStateActions albumId={album.id} />{artist.albumCount === 1 ? <Suspense fallback={null}><ArtistCollectionExperience artist={artist} inline /></Suspense> : null}</article>)}</div></section>)}</div>
    </section>
