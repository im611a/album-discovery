...
export function RecommendationCatalog() {
  ...
  return <>
    ...
    {recommendations.length ? <>
      <section className="catalog-section" aria-labelledby="familiar-title"><header className="section-heading"><div><p className="section-kicker">与你当前偏好重合</p><h2 id="familiar-title">熟悉方向</h2></div></header><div className="recommendation-grid">{familiar.map((item) => <AlbumCard key={item.album.id} album={item.album} reason={item.reasons.join(" ")} />)}</div></section>
      <section className="catalog-section" aria-labelledby="adjacent-title"><header className="section-heading"><div><p className="section-kicker">保留一部分共同信号</p><h2 id="adjacent-title">试试相邻方向</h2></div></header>{adjacent.length ? <div className="recommendation-grid">{adjacent.map((item) => <AlbumCard key={item.album.id} album={item.album} reason={item.reasons.join(" ")} />)}</div> : <p className="empty-state">给几张专辑标记“喜欢”，这里会出现更有根据的拓展选择。</p>}</section>
