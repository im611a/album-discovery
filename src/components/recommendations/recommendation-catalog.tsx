...
export function RecommendationCatalog() {
  ...
  return <>
    <section className="taste-bar" aria-labelledby="taste-summary-title"><div><p className="section-kicker">你的口味</p><h2 id="taste-summary-title">{state.taste.genres.length ? state.taste.genres.map(getTaxonomyLabel).join("、") : "开放探索"}</h2><p>{state.taste.contexts.length ? `常见场景：${state.taste.contexts.join("、")}` : "暂未选择固定场景"} · 反馈只保存在本机</p></div><Link className="button button--secondary" href="/settings#taste">编辑口味</Link></section>
    {recommendations.length ? <>
      <section className="catalog-section" aria-labelledby="familiar-title"><header className="section-heading"><div><p className="section-kicker">与你当前偏好重合</p><h2 id="familiar-title">熟悉方向</h2></div></header><div className="recommendation-grid">{familiar.map((item) => <AlbumCard key={item.album.id} album={item.album} reason={item.reasons.join(" ")} actions="full" />)}</div></section>
      <section className="catalog-section" aria-labelledby="adjacent-title"><header className="section-heading"><div><p className="section-kicker">保留一部分共同信号</p><h2 id="adjacent-title">试试相邻方向</h2></div></header>{adjacent.length ? <div className="recommendation-grid">{adjacent.map((item) => <AlbumCard key={item.album.id} album={item.album} reason={item.reasons.join(" ")} actions="full" />)}</div> : <p className="empty-state">给几张专辑标记“喜欢”，这里会出现更有根据的拓展选择。</p>}</section>
