...
function SearchCatalogView({ query, results, router }: { query: string; results: ReturnType<typeof searchAlbums>; router: ReturnType<typeof useRouter> }) {
  ...
  return <>
    ...
    {!query ? <div className="empty-state empty-state--initial"><h2>从一个名字或感觉开始</h2><p>例如“王菲”“ambient”“夜晚”或“朦胧”。</p></div> : <><div className="results-bar" aria-live="polite"><p>“{query}”找到 <strong>{results.length}</strong> 张专辑</p></div>{results.length ? <AlbumGrid albums={results} /> : <div className="empty-state"><h2>没有找到匹配专辑</h2><p>试试缩短关键词，或改用艺术家、流派和场景词。</p></div>}</>}
  </>;
}
