...
function SearchCatalogView({ query, results, page, artists, router }: { query: string; results: ReturnType<typeof searchAlbums>; page: PageSlice<PublishedAlbumSummary>; artists: ReturnType<typeof searchArtists>; router: ReturnType<typeof useRouter> }) {
  ...
  return <>
    <form className="search-form pa-search-instrument" role="search" data-search-state={query ? "results" : "idle"} onSubmit={submit}><label htmlFor="catalog-search">搜索专辑目录</label><div><span className="pa-search-instrument__index" aria-hidden="true">Q—</span><input id="catalog-search" name="q" value={value} onChange={(e) => setValue(e.target.value)} placeholder="专辑、别名或艺术家" autoComplete="off" /><button className="button button--primary" type="submit">检索档案</button>{query ? <button className="button button--quiet" type="button" onClick={() => { setValue(""); router.push("/search", { scroll: false }); }}>清空</button> : null}</div></form>
