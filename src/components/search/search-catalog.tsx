...
function SearchCatalogView({ query, results, router }: { query: string; results: ReturnType<typeof searchAlbums>; router: ReturnType<typeof useRouter> }) {
  ...
  return <>
    <form className="search-form" role="search" onSubmit={submit}><label htmlFor="catalog-search">搜索专辑目录</label><div><input id="catalog-search" name="q" value={value} onChange={(e) => setValue(e.target.value)} placeholder="专辑、别名、艺术家、流派或氛围特征" autoComplete="off" /><button className="button button--primary" type="submit">搜索</button></div></form>
