...
export function SearchCatalog() {
  ...
  return <SearchCatalogView key={query} query={query} results={results} artists={artists} router={router} />;
}

function SearchCatalogView({ query, results, artists, router }: { query: string; results: ReturnType<typeof searchAlbums>; artists: ReturnType<typeof searchArtists>; router: ReturnType<typeof useRouter> }) {
  ...
  return <>
    ...
    {!query ? <div className="empty-state empty-state--initial"><h2>从一个名字开始</h2><p>可以搜索中文或英文专辑名、已登记别名与艺人名称。</p></div> : <><div className="results-bar" aria-live="polite"><p>“{query}”找到 <strong>{results.length}</strong> 张专辑与 <strong>{artists.length}</strong> 位艺人</p></div>{artists.length ? <section className="search-artist-results" aria-labelledby="artist-results-title"><h2 id="artist-results-title">艺人</h2><div className="artist-grid artist-grid--search">{artists.slice(0, 6).map((artist) => <ArtistCard key={artist.artistId} artist={artist} />)}</div></section> : null}{results.length ? <section className="search-album-results" aria-labelledby="album-results-title"><h2 id="album-results-title">专辑</h2><AlbumGrid albums={results} highlight={query} /></section> : null}{!results.length && !artists.length ? <div className="empty-state"><h2>没有找到匹配内容</h2><p>试试缩短关键词，或换用专辑别名和艺人名称。</p><Link className="button button--secondary" href="/discover">前往发现页</Link></div> : null}</>}
  </>;
}
