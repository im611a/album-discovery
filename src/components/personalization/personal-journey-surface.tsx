...
export function PersonalJourneySurface({ context, source, title, eyebrow, className, currentAlbumSlug, originArtistSlug, currentAlbumIds, eligibleAlbumIds, relationFallbackAlbumIds, limit = 8, forYou = false, onAdjustTaste }: {
  ...
  return <PersonalJourneySection presentation={presentation} title={title} eyebrow={eyebrow} className={className} forYou={forYou} activeIndex={activeIndex} onNext={() => setActiveIndex((index) => index + 1)} onAdjustTaste={onAdjustTaste} />;
}
