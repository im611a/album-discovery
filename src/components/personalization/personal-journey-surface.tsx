...
export function PersonalJourneySurface({ context, source, title, eyebrow, className, currentAlbumSlug, currentAlbumIds, eligibleAlbumIds, relationFallbackAlbumIds, limit = 8 }: {
  ...
  const presentation = useMemo(() => hydrated ? buildPersonalJourneyPresentation({
    state, context, source, limit, searchParams: query, currentAlbumSlug,
    currentAlbumIds, eligibleAlbumIds, relationFallbackAlbumIds,
  }) : null, [context, currentAlbumIds, currentAlbumSlug, eligibleAlbumIds, hydrated, limit, query, relationFallbackAlbumIds, source, state]);
