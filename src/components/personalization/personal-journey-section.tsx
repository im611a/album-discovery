...
function JourneyCard({ option, emphasis = false }: { option: PersonalJourneyOption; emphasis?: boolean }) {
  return <article className="r14-journey-card" data-provenance={option.provenance} data-emphasis={emphasis ? "primary" : "supporting"}>
    <Link className="r14-journey-card__cover" href={option.href} aria-label={`查看《${option.album.title}》专辑导览：${option.lens}`}>
    ...
    <div className="r14-journey-card__copy">
      ...
      <h3><Link href={option.href}>{option.album.title}</Link></h3>
      ...
      <Link className="r14-journey-card__continue" href={option.href}>沿这条线索继续 <span aria-hidden="true">→</span></Link>
