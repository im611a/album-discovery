  ...
  return <article className="artist-card">
    <Link href={`/artists/${artist.slug}`} className="artist-card__link">
      <div className="artist-card__covers" aria-hidden="true">
        {artist.previewCovers.length ? artist.previewCovers.slice(0, 1).map((cover) => <Image key={cover} src={withBasePath(cover)} width={160} height={160} alt="" loading="lazy" unoptimized />) : <span>{artist.name.slice(0, 1)}</span>}
      </div>
