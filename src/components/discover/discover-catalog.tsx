...
export function DiscoverCatalog() {
  ...
  return <>
    <details className="filter-panel"><summary>筛选与排序 <span>{Object.values(filters).filter(Boolean).length} 项</span></summary><div className="filter-grid">
      ...
      <label>聆听场景（本站策展）<select value={filters.context ?? ""} onChange={(e) => update("context", e.target.value)}><option value="">全部</option>{options.contexts.map((value) => <option key={value} value={value}>{value}</option>)}</select></label>
