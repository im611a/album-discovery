...
export function DiscoverCatalog() {
  ...
  return <>
    <details className="filter-panel"><summary>筛选与排序 <span>{Object.values(filters).filter(Boolean).length} 项</span></summary><div className="filter-grid">
      ...
      {options.relatedGenres.length ? <label>延伸流派<select value={filters.relatedGenre ?? ""} onChange={(e) => update("secondary", e.target.value)}><option value="">全部</option>{options.relatedGenres.map((value) => <option key={value} value={value}>{getTaxonomyLabel(value)}</option>)}</select></label> : null}
