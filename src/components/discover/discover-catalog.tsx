...
export function DiscoverCatalog() {
  ...
  return <>
    <details className="filter-panel"><summary>筛选与排序 <span>{Object.values(filters).filter(Boolean).length} 项</span></summary><div className="filter-grid">
      ...
      {options.relatedGenres.length ? <label>相关流派<select value={filters.relatedGenre ?? ""} onChange={(e) => update("secondary", e.target.value)}><option value="">全部</option>{options.relatedGenres.map((value) => <option key={value} value={value}>{getTaxonomyLabel(value)}</option>)}</select></label> : null}
      {options.descriptors.length ? <label>氛围与特征<select value={filters.descriptor ?? ""} onChange={(e) => update("descriptor", e.target.value)}><option value="">全部</option>{options.descriptors.map((value) => <option key={value} value={value}>{getDescriptorLabel(value)}</option>)}</select></label> : null}
