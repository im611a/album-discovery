...
function TasteSetupForm({ initialTaste, embedded, redirectTo, onSave }: { initialTaste: TasteProfile; embedded: boolean; redirectTo: string | null; onSave: (taste: TasteProfile, completed?: boolean) => void }) {
  ...
  return <section className={embedded ? "taste-setup taste-setup--embedded" : "taste-setup"} aria-labelledby="taste-title">
    ...
    <fieldset><legend>核心流派偏好 <span>{signals}/5</span></legend><div className="choice-grid">{coreTaxonomy.map((item) => <button type="button" key={item.key} aria-pressed={taste.genres.includes(item.key)} onClick={() => updateGenre(item.key)}>{getTaxonomyLabel(item.key)}</button>)}</div></fieldset>
    <fieldset><legend>常见聆听场景 <span>本站策展维度</span></legend><div className="choice-grid choice-grid--compact">{contexts.map((context) => <button type="button" key={context} aria-pressed={taste.contexts.includes(context)} onClick={() => setTaste((current) => ({ ...current, contexts: toggle(current.contexts, context, 4) }))}>{getListeningSceneLabel(context)}</button>)}</div></fieldset>
