...
import { catalogAlbums, catalogTaxonomy, descriptorTaxonomy, getDescriptorLabel, getTaxonomyLabel } from "@/catalog/published-catalog";
...
const descriptorChoices = descriptorTaxonomy.map((item) => item.key);
const availableDescriptorKeys = new Set(descriptorChoices);
...
function TasteSetupForm({ initialTaste, embedded, redirectTo, onSave }: { initialTaste: TasteProfile; embedded: boolean; redirectTo: string | null; onSave: (taste: TasteProfile, completed?: boolean) => void }) {
  ...
  const signals = taste.genres.length + taste.descriptors.filter((key) => availableDescriptorKeys.has(key)).length;
  const summary = useMemo(() => [taste.genres.map(getTaxonomyLabel).join("、"), taste.descriptors.filter((key) => descriptorChoices.includes(key)).map(getDescriptorLabel).join("、"), taste.contexts.join("、"), taste.eras.map((era) => era.replace("s", " 年代")).join("、"), taste.exploration === "familiar" ? "偏熟悉" : taste.exploration === "exploratory" ? "偏探索" : "熟悉与探索平衡"].filter(Boolean).join(" · "), [taste]);
  ...
  const updateSignal = (key: "genres" | "descriptors", value: string) => setTaste((current) => {
    const currentSignals = current.genres.length + current.descriptors.filter((item) => availableDescriptorKeys.has(item)).length;
  ...
  return <section className={embedded ? "taste-setup taste-setup--embedded" : "taste-setup"} aria-labelledby="taste-title">
    ...
    <fieldset><legend>核心流派偏好 <span>{signals}/5</span></legend><div className="choice-grid">{coreTaxonomy.map((item) => <button type="button" key={item.key} aria-pressed={taste.genres.includes(item.key)} onClick={() => updateSignal("genres", item.key)}>{getTaxonomyLabel(item.key)}</button>)}</div></fieldset>
    {descriptorChoices.length ? <fieldset><legend>氛围与特征</legend><div className="choice-grid choice-grid--compact">{descriptorChoices.map((key) => <button type="button" key={key} aria-pressed={taste.descriptors.includes(key)} onClick={() => updateSignal("descriptors", key)}>{getDescriptorLabel(key)}</button>)}</div></fieldset> : null}
