...
const descriptors = ["层次丰富", "朦胧", "空间感", "旋律性", "叙事性", "沉浸", "即兴", "律动", "亲密", "强劲", "明亮", "缓慢"];
...
function TasteSetupForm({ initialTaste, embedded, redirectTo, onSave }: { initialTaste: TasteProfile; embedded: boolean; redirectTo: string | null; onSave: (taste: TasteProfile, completed?: boolean) => void }) {
  ...
  const summary = useMemo(() => [taste.genres.map((key) => catalogTaxonomy.find((item) => item.key === key)?.labelZh).filter(Boolean).join("、"), taste.descriptors.join("、"), taste.contexts.join("、"), taste.eras.map((era) => era.replace("s", " 年代")).join("、"), taste.exploration === "familiar" ? "偏熟悉" : taste.exploration === "exploratory" ? "偏探索" : "熟悉与探索平衡"].filter(Boolean).join(" · "), [taste]);
  ...
  return <section className={embedded ? "taste-setup taste-setup--embedded" : "taste-setup"} aria-labelledby="taste-title">
    ...
    <fieldset><legend>类型偏好 <span>{signals}/5</span></legend><div className="choice-grid">{catalogTaxonomy.map((item) => <button type="button" key={item.key} aria-pressed={taste.genres.includes(item.key)} onClick={() => updateSignal("genres", item.key)}>{item.labelZh}</button>)}</div></fieldset>
    <fieldset><legend>声音描述</legend><div className="choice-grid choice-grid--compact">{descriptors.map((item) => <button type="button" key={item} aria-pressed={taste.descriptors.includes(item)} onClick={() => updateSignal("descriptors", item)}>{item}</button>)}</div></fieldset>
