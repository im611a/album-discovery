...
export function AlbumDetail({ album }: { album: PublishedAlbum }) {
  ...
  return <article className="album-detail">
    ...
    <div className="album-detail__content">
      ...
      {related.length ? <section className="related-section" aria-labelledby="related-title"><header className="section-heading"><div><p className="section-kicker">相同类型或聆听信号</p><h2 id="related-title">继续发现</h2></div></header><AlbumGrid albums={related} headingLevel={3} /></section> : null}
    </div>
  </article>;
}
