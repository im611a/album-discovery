...
export function AlbumDetail({ album: suppliedAlbum, viewModel, sameArtistAlbums = [] }: AlbumDetailProps) {
  ...
  return <article className="album-detail pa-album-file r12-album-object">
    ...
    <div className="album-detail__content">
      ...
      {resolvedViewModel.recommendations.length ? <section className="related-section pa-album-recommendations" aria-labelledby="album-recommendations-title"><DetailSectionHeading number="05B" kicker="基于目录字段" title="推荐专辑" id="album-recommendations-title" /><p className="support-copy">按核心流派、可靠相关流派与本站聆听场景的既有顺序呈现，不使用热度或远程请求。</p><AlbumGrid albums={resolvedViewModel.recommendations} headingLevel={3} /></section> : null}
