...
export function TopicPage({ topic, pathname }: { topic: TopicSummary; pathname: string }) {
  ...
  return <article className="pa-topic-file" data-topic-kind={topic.kind}>
    ...
    {topic.commonCoreGenres.length ? <div className="topic-common"><span>常见核心流派</span>{topic.commonCoreGenres.map((item) => <ReturnContextLink key={item.key} href={`/discover?core=${encodeURIComponent(item.key)}`}>{getTaxonomyLabel(item.key)} · {item.count}</ReturnContextLink>)}</div> : null}
